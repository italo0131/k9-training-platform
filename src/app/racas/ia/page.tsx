...
import SectionAutoFocus from "@/app/racas/SectionAutoFocus"
import { loadBreedExplorerProfiles } from "@/app/racas/explorer-data"

export const revalidate = 21600

export default async function RacasIaPage() {
  ...
  return (
    <div className="min-h-[100svh] bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.14),transparent_26%),radial-gradient(circle_at_80%_20%,rgba(244,114,182,0.12),transparent_24%),linear-gradient(145deg,#020617,#08111f_45%,#020617)] px-4 py-8 text-white sm:px-6">
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-[34px] border border-white/10 bg-[linear-gradient(135deg,rgba(30,41,59,0.92),rgba(15,23,42,0.94)_50%,rgba(59,7,100,0.56)),radial-gradient(circle_at_top_right,rgba(56,189,248,0.16),transparent_30%)] p-6 md:p-8">
          <Link
            href="/racas"
          ...
          <div className="mt-6 max-w-4xl space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-500/10 px-3 py-1 text-xs uppercase tracking-[0.2em] text-cyan-100">
              ...
              <span>Consultor com IA</span>
            </div>
            <h1 className="text-4xl font-semibold leading-tight md:text-5xl">
              Use a IA para organizar sua decisão de raça
            </h1>
            <p className="text-base leading-7 text-slate-200 md:text-lg">
              Selecione algumas raças, descreva seu contexto e peça uma leitura comparativa em linguagem direta.
            </p>
          </div>
        </section>

        <SectionAutoFocus sectionId="coach-racas" />
        <BreedExplorer profiles={profiles} errorMessage={errorMessage} pageTitle="Consultor com IA" />
      </div>
    </div>
  )
}
